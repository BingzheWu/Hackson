...
bool sort_by_x(const cv::Point2i a, const cv::Point2i b){
	return a.x < b.x;
}

void HorizonLineDetector::draw_ori(cv::Mat frame, double factor)
{
    const cv::Scalar s1(0,255,0);
    //cvtColor(current_frame,current_draw, CV_GRAY2RGB);
	...
	//for (size_t i=0;i<horizon.size();i++)
	//	printf("%d %d ", horizon[i].x, horizon[i].y);
	//printf("\n");

	sort(horizon.begin(), horizon.end(), sort_by_x);

    for (size_t i=0;i<horizon.size()-1;i++)
    {
		int x1 = horizon[i].x * factor;
		int y1 = horizon[i].y * factor;
		int x2 = horizon[i+1].x * factor;
		int y2 = horizon[i+1].y * factor;
		//printf("%d %d %d %d %f\n", x1,y1,x2,y2, factor);
		...
		for( int j = x1; j <= x2; ++j){
			int tmp = y1 + (j-x1) * k;
			//printf("%d %d\n\n", j, tmp);
			if(  0 <= j && j + 1 < frame.cols && 0 <= tmp && tmp + 2 < frame.rows )
				frame.colRange(j, j + 1)
     	             .rowRange(tmp, tmp + 2)=s1;
		}
    }
}

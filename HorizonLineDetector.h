...
struct FaceResult{
    ...
    FaceResult(CvRect _face, int _w, int _d){
        face = _face;
        weight = _w;
        dir = _d;
    }
...
void getHoughSil(std::vector<cv::Vec4i> lines, cv::Mat& image, double thresh1, double thresh2);
...
class HorizonLineDetector
{
	public:
        ...
        void draw_horizon();
		void draw_ori(cv::Mat frame, double factor);
        void draw_edges();
